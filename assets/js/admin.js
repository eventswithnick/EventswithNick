(function () {
  ...
  const ADMIN_VERSION = 'v7.4 Stable Admin';
  ...
  async function showDashboard() {
    ...
    const status = document.getElementById('connectionStatus');
    if (status) status.textContent = hasRemote() ? `Secure database connected${currentAdmin && currentAdmin.email ? ': ' + currentAdmin.email : ''} • ${ADMIN_VERSION}` : `Presentation mode (local only) • ${ADMIN_VERSION}`;
    systemNotice('', 'info');
